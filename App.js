...
import { Platform, View, Modal, Text, TouchableOpacity, Linking, StyleSheet, Animated, PanResponder, Dimensions } from 'react-native';
...
function WhatsAppFAB() {
  const pan = useRef(new Animated.ValueXY({
    x: Dimensions.get('window').width - FAB_SIZE - 20,
    y: Dimensions.get('window').height - FAB_SIZE - 120,
  })).current;
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderRelease: () => {
        ...
        const x = Math.max(4, Math.min(pan.x._value, width - FAB_SIZE - 4));
        const y = Math.max(40, Math.min(pan.y._value, height - FAB_SIZE - 20));
        Animated.spring(pan, { toValue: { x, y }, useNativeDriver: false, friction: 6 }).start();
        if (!dragged.current) handlePress();
      },
    })
  ).current;
  ...
  return (
    <Animated.View
      ...
      style={[waStyles.fab, { transform: pan.getTranslateTransform() }]}>
